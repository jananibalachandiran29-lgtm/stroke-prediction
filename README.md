...
`model.pkl` is regenerated by `train.py` in the current project format. The prediction application automatically retrains if the existing model file is missing or is in an older incompatible format.
